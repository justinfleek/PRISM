...
/**
 * PRISM 211° Monochromatic Color Generator
 * 
 * The core philosophy: from a single hue, generate a beautiful monochromatic
 * palette by varying only LIGHTNESS and CHROMA - never the hue itself.
 * 
 * Named after 211° - the perfect blue. All colors in the palette share
 * the same hue, creating harmonious gradients.
 * 
 * MONOCHROMATIC APPROACH:
 * - All 16 colors use the SAME hue
 * - Backgrounds: low chroma, varying lightness
 * - Foregrounds: low chroma, higher lightness (dark mode) / lower lightness (light mode)
 * - Accents: HIGH chroma, same hue - the hero color pops!
 * - Semantic meaning comes from LIGHTNESS and CHROMA, not hue shifts
 */

// ============================================================================
// SEMANTIC COLOR DEFINITIONS (Monochromatic)
// ============================================================================
...
export interface SemanticColor {
  ...
  name: string;
  description: string;
  why: string;  // Explains the semantic reasoning
  lightnessTarget: { dark: number; light: number };
  chromaMultiplier: number;  // 0 = gray, 1 = full saturation
...
// ============================================================================
// PALETTE GENERATION (Monochromatic)
// ============================================================================

export interface GeneratorConfig {
  heroHue: number;         // 0-360 (default: 211)
  heroChroma: number;      // 0-1 (default: 1.0) - how saturated the accents are
  mode: "dark" | "light";
  monitor: "oled" | "lcd";
  name: string;
}
...
export interface GeneratedPalette {
  colors: Record<string, string>;  // base00-0F hex values
  contrast: {
    text: number;
    comment: number;
    accent: number;
 ...
 */
export function generateMonochromaticPalette(config: GeneratorConfig): GeneratedPalette {
  ...
  for (const sc of SEMANTIC_COLORS) {
    ...
    if (sc.role === "foreground" && sc.id !== "base04") {
      const bg = hexToSrgb(colors.base00 || srgbToHex(oklchToSrgb({ L: baseBgL, C: maxChroma * 0.08 * 0.3, H: heroHue })));
    ...
    colors[sc.id] = srgbToHex(rgb);
  }
  
  // Calculate contrast ratios
  const bg = hexToSrgb(colors.base00);
  const crText = contrastRatio(hexToSrgb(colors.base05), bg);
  const crComment = contrastRatio(hexToSrgb(colors.base03), bg);
  const crAccent = contrastRatio(hexToSrgb(colors.base0A), bg);
...
export class Prism211GeneratorPanel {
  public static currentPanel: Prism211GeneratorPanel | undefined;
  public static readonly viewType = "prism211Generator";

  private readonly _panel: vscode.WebviewPanel;
  private readonly _context: vscode.ExtensionContext;
  private _disposables: vscode.Disposable[] = [];
  private _currentPalette: GeneratedPalette | null = null;
  ...
  public static createOrShow(context: vscode.ExtensionContext) {
    ...
    const panel = vscode.window.createWebviewPanel(
      Prism211GeneratorPanel.viewType,
      "Prism 211° Generator",
      column,
      {
        enableScripts: true,
  ...
  private _generatePalette(config: GeneratorConfig) {
  ...
  private _updateColor(colorId: string, hex: string) {
    if (this._currentPalette) {
      ...
      if (["base00", "base03", "base05", "base0A"].includes(colorId)) {
        const bg = hexToSrgb(this._currentPalette.colors.base00);
        const text = hexToSrgb(this._currentPalette.colors.base05);
        const comment = hexToSrgb(this._currentPalette.colors.base03);
        const accent = hexToSrgb(this._currentPalette.colors.base0A);
        
        this._currentPalette.contrast = {
          text: Math.round(contrastRatio(text, bg) * 100) / 100,
          comment: Math.round(contrastRatio(comment, bg) * 100) / 100,
          accent: Math.round(contrastRatio(accent, bg) * 100) / 100,
          wcagVerified: wcagAA(contrastRatio(text, bg)) && 
                        wcagAALarge(contrastRatio(comment, bg)) && 
                        wcagAALarge(contrastRatio(accent, bg)),
        };
  ...
  private async _exportTheme(name: string, colors: Record<string, string>) {
  ...
  private async _applyTheme(name: string, colors: Record<string, string>) {
    const theme = this._buildVSCodeTheme(name, colors);
  ...
  private _buildVSCodeTheme(name: string, c: Record<string, string>): object {
    return {
      name: `Prism ${name}`,
      type: "dark",
      colors: {
  ...
  private _getHtmlForWebview(): string {
    const semanticColorsJSON = JSON.stringify(SEMANTIC_COLORS);
...
<body>
  <h1>Prism <span>211</span> Generator</h1>
  ...
  <footer class="attribution">
    <span>Built by</span>
    <a href="https://fleek.sh" target="_blank">Fleek</a>
  </footer>
